 ...
 */
const results = [];
async function mockHandleTool(name, args) {
    results.push(`Executed: ${name} with ${JSON.stringify(args)}`);
}
async function deepSniff(content) {
...
async function runProof() {
    ...
    if (results.some(r => r.includes('write_file')))
        console.log('✅ Success: Handled Gemini nested array.');
    ...
    if (results.some(r => r.includes('generate_sprite')))
        console.log('✅ Success: Handled text-wrapped object.');
...
runProof();
//# sourceMappingURL=test-deep-sniff.js.map